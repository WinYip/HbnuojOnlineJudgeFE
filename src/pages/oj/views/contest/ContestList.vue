<template>
  <Row type="flex">
    ...
    <Panel id="contest-card" shadow>
        ...
        <ul class="filter">
          <li>
            <Dropdown @on-click="onRuleChange">
              <span>{{query.rule_type === '' ? '赛制' : query.rule_type}}
      ...
      <p id="no-contest" v-if="contests.length == 0">暂无比赛</p>
      <ol id="contest-list">
        <li v-for="contest in contests" :key="contest.title">
          <Row type="flex" justify="space-between" align="middle">
            ...
            <Tag type="dot" :color="CONTEST_STATUS_REVERSE[contest.status].color">
              {{contest_status_name(CONTEST_STATUS_REVERSE[contest.status].name)}}
              </Tag>
            </Col>
          </Row>
        </li>
